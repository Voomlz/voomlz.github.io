...
async function fetchWCLreport(path, start, end) {
  ...
  let filter = encodeURI(
    `type IN ("death","cast","begincast") OR ability.id IN (${Object.keys(
      notableBuffs
    ).join(
      ","
    )}) OR (type IN ("damage","heal","healing","miss","applybuff","applybuffstack","refreshbuff","applydebuff","applydebuffstack","refreshdebuff", "resourcechange","resourcechange","absorbed","healabsorbed","leech","drain", "removebuff") AND ability.id NOT IN (${zeroThreatSpells.join(
      ","
    )}))`
  );
...
class ThreatTrace {
  ...
  tabulate(el_div) {
    el_div.innerHTML =
      ...
      parseFloat(this.target.initialCoeff.toFixed(4));
    // Color
    ...
    for (let k in a) {
      ...
      els[1].textContent =
        buffNames[k] + " " + buffMultipliers[k](this.target.spellSchool);
...
class Unit {
  constructor(key, name, type, events, lastInvisibility = 0) {
    ...
    this.baseThreatCoeff =
      baseThreatCoefficients[type] || getThreatCoefficient(1);
      ...
      if (combatantInfoElement.sourceID == this.key) {
        ...
        if (gear) {
          for (const g of gear) {
            ...
            if (enchant != null) {
              if (enchant === 2613) {
                // gloves threat enchant
                this.buffs[2613] = true;
              }
              if (enchant === 2621) {
                // cloack threat enchant
...
class NPC extends Unit {
  ...
  plot(reportId, fight) {
    ...
    createCheckbox(
      el_div,
      splitHealingThreatOption,
      "Split healing threat",
      (x) => {
...
class Fight {
  ...
  async fetch() {
    combatantInfo = await fetchWCLCombatantInfo(
      this.reportId + "?",
      this.start,
      this.end
    );
...
function selectEnemy() {
  try {
    ...
    for (let k in u.threat) {
      ...
      el_u.value = s + ";" + k;
      el_u.textContent =
        u.threat[k].target.name + " - " + u.threat[k].target.key;
...
function selectTarget(target) {
  try {
    ...
    reports[reportId].fights[fightId].enemies[enemyId].threat[
      targetId
    ].tabulate(el_out);
  } catch (e) {
    printError(e);
