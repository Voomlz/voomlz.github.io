...
async function fetchWCLreport(path, start, end) {
  ...
  let filter = encodeURI(
    `type IN ("death","cast","begincast") OR ability.id IN (${Object.keys(
      ...
      ","
    )}) OR (type IN ("damage","heal","healing","miss","applybuff","applybuffstack","refreshbuff","applydebuff","applydebuffstack","refreshdebuff","resourcechange","absorbed","healabsorbed","leech","drain", "removebuff") AND ability.id NOT IN (${zeroThreatSpells.join(
      ","
    )}))`
  );
...
class ThreatTrace {
  ...
  tabulate(el_div) {
    ...
    for (let k in a) {
      ...
      const coefficient =
        typeof buffMultipliers[k] === "function"
          ? buffMultipliers[k](this.target.spellSchool)
          : "";
...
class Unit {
  constructor(key, name, type, events) {
  ...
  threatCoeff(ability) {
    ...
    for (let i in this.buffs) {
      if (i in buffMultipliers) {
        ...
        if (
          typeof buffMultipliers[i] === "object" &&
          spellId &&
          "coeff" in buffMultipliers[i]
        ) {
