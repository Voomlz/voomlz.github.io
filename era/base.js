...
/**
 * @typedef {{value: number, label: string, hasBonus?: boolean}} CoefficientDebug
 */
 ...
 * @param {number} value
 * @param {string} label
 * @param {boolean} [hasBonus] Force display even if the coefficient is 1, because it may have a bonus
 * @returns {ThreatCoefficient}
 */
export function applyThreatCoefficient(coeff, value, label, hasBonus) {
  return {
    value: coeff.value * value,
    debug: [...coeff.debug, { value, label, hasBonus }],
  };
}
...
export function handler_modDamagePlusThreat(multiplier, bonus) {
  return (ev, fight) => {
    ...
    threatFunctions.sourceThreatenTarget({
      ...
      multiplier,
      bonusThreat: bonus,
      debugLabel: `modDamagePlusThreat (dmg * ${multiplier}) + ${bonus}`,
    });
  };
}
...
export function handler_threatOnHit(threatValue) {
  return (ev, fight) => {
    ...
    threatFunctions.sourceThreatenTarget({
      ...
      debugLabel: `threatOnHit: dmg + ${threatValue}`,
    });
  };
}
...
export function handler_threatOnDebuff(threatValue) {
  return (ev, fight) => {
    ...
    threatFunctions.sourceThreatenTarget({
      ...
      fight,
      amount: threatValue,
      debugLabel: `threatOnDebuff: ${threatValue}`,
    });
  };
}
  ...
  return (ev, fight) => {
    ...
    if (t === "applydebuff") {
      threatFunctions.sourceThreatenTarget({
        ...
        fight,
        amount: threatValue,
        debugLabel: `threatOnDebuffOrDamage: ${threatValue}`,
      });
    } else if (t === "damage") {
      threatFunctions.sourceThreatenTarget({
        ev,
        fight,
        amount: ev.amount + (ev.absorbed || 0),
        debugLabel: `threatOnDebuffOrDamage: dmg`,
      });
    }
  };
}
...
export function handler_threatOnBuff(threatValue) {
  return (ev, fight) => {
    ...
    threatFunctions.unitThreatenEnemiesSplit({
      ...
      fight,
      amount: threatValue,
      debugLabel: `threatOnBuff: ${threatValue}`,
    });
  };
}
