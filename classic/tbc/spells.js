...
export const auraImplications = {
  ...
  Warrior: {
    ...era.auraImplications.Warrior,
    // 6343: undefined,
    // 8198: undefined,
    // 8204: undefined,
    // 8205: undefined,
    // 11580: undefined,
    // 11581: undefined, //Thunderclap now usable in def stance
    // 12292: undefined, //Sweeping Strikes now usable in berserker stance
    // 30357: 71, //Revenge
...
export const spellFunctions = {
  ...
  [GLOBAL_SPELL_HANDLER_ID]: threatFunctions.concat(handler_nightbaneLanding),
  ...
  37098: handler_nightbaneThreatWipeAfterDelay(43 * 1000), // Nightbane's Rain of Bones. delay : 43 sec is the timer according to DBM
...
function unitThreatenEnemiesSplitOnHealRedirect({
  ...
  if (globalThis.splitHealingThreatOption) {
    let numEnemies = 0;
...
// hack for nightbane
function handler_nightbaneLanding(ev, fight) {
...
function handler_nightbaneThreatWipeAfterDelay(delayMs) {
  return (ev, fight) => {
    if (ev.type !== "cast") return;
    let u = fight.eventToUnit(ev, "source");
    nightBaneNextLanding = ev.timestamp + delayMs;
    if (!u) return;
    for (let k in u.threat) {
